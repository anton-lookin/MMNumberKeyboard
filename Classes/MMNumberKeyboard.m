...
- (void)_configureButtonsForCurrentStyle
{
	...
	UIButton *specialButton = [_MMNumberKeyboardButton keyboardButtonWithStyle:MMNumberKeyboardButtonStyleDone];
...
- (void)layoutSubviews
{
	...
	for (MMNumberKeyboardButton key = numberMin; key < numberMax; key++) {
		...
		if (digit == 0) {
			...
			if (!allowsDecimalPoint) {
				rect.size.width = numberSize.width * 3.0f;
			}
	...
	const int utilityButtonKeys[2] = { MMNumberKeyboardButtonBackspace, MMNumberKeyboardButtonSpecial };
	...
	for (NSInteger idx = 0; idx < sizeof(utilityButtonKeys) / sizeof(int); idx++) {
		...
		_MMNumberKeyboardButton *button = buttonDictionary[@(key)];
		...
		if (key == MMNumberKeyboardButtonSpecial) {
			CGFloat inset = button.frame.size.width * 0.35;
			UIView *buttonView = [[UIView alloc] initWithFrame:CGRectMake(0.0f,
																		  0.0f,
																		  button.frame.size.width,
																		  button.frame.size.height)];
			button.customImageView = [[UIImageView alloc] initWithImage:button.image];
			button.customImageView.tintColor = [UIColor whiteColor];
			button.customImageView.contentMode = UIViewContentModeScaleAspectFit;
			button.customImageView.frame = CGRectMake(inset,
													  inset,
													  button.frame.size.width - inset * 2,
													  button.frame.size.height * 0.5f - inset);
			button.customTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0.0f,
																				button.frame.size.height * 0.55f,
																				button.frame.size.width,
																				20.0f)];
			[button.customTitleLabel setText:@"Scan"];
			button.customTitleLabel.textAlignment = NSTextAlignmentCenter;
			button.customTitleLabel.textColor = [UIColor whiteColor];
			[buttonView addSubview:button.customImageView];
			[buttonView addSubview:button.customTitleLabel];
...
- (void)_updateButtonAppearance
{
	if (self.isHighlighted || self.isSelected) {
		...
		self.customImageView.tintColor = self.highlightedControlColor;
		self.customTitleLabel.textColor = self.highlightedControlColor;
	} else {
		self.backgroundColor = self.fillColor;
		self.imageView.tintColor = self.highlightedControlColor;
		self.customImageView.tintColor = self.controlColor;
		self.customTitleLabel.textColor = self.controlColor;
	}
}
